...
export const createWebhook = async (req: Request, res: Response) => {
  try {
    const { url, name, method, body } = req.body;
    if (!url || !method || !name) throw new MissingRequiredFieldsError();
...
export const updateWebhook = async (req: Request, res: Response) => {
  try {
    const { id, url, name, method, body } = req.body;
    if (!id || !url || !method || !name) throw new MissingRequiredFieldsError();
